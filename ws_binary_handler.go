...
func (c *Client) handleBinaryUpload(message []byte) {
	...
	if err := json.Unmarshal(message, &uploadMsg); err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al decodificar mensaje: %v", err))
		return
	}

	// Validar datos
	if uploadMsg.Collection == "" {
		c.sendErrorMessage("Se requiere una colección")
		return
	}
	if uploadMsg.Filename == "" {
		c.sendErrorMessage("Se requiere un nombre de archivo")
		return
	}
	if uploadMsg.Data == "" {
		c.sendErrorMessage("Se requieren datos")
		return
	}
	...
	if err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al decodificar datos base64: %v", err))
		return
	}
	...
	if err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al crear archivo temporal: %v", err))
		return
	}
	...
	if _, err := tempFile.Write(data); err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al escribir datos: %v", err))
		return
	}

	// Rebobinar el archivo
	if _, err := tempFile.Seek(0, io.SeekStart); err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al rebobinar archivo: %v", err))
		return
	}
	...
	if binaryManager == nil {
		c.sendErrorMessage("Gestor de binarios no disponible")
		return
	}

	fileMetadata, err := binaryManager.StoreFile(tempFile, uploadMsg.Filename, mimeType, options...)
	if err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al almacenar archivo: %v", err))
		return
	}
...
func (c *Client) handleBinaryDownload(message []byte) {
	...
	// Validar datos
	if downloadReq.ID == "" {
		c.sendErrorMessage("Se requiere un ID")
		return
	}
	...
	if err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al recuperar archivo: %v", err))
		return
	}
	...
	if err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al leer datos: %v", err))
		return
	}
...
func (c *Client) handleBinaryList(message []byte) {
	...
	if err != nil {
		c.sendErrorMessage(fmt.Sprintf("Error al listar archivos: %v", err))
		return
	}
